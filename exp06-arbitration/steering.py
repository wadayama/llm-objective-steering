"""System 2 plumbing for exp06: arbitration conditions on top of the exp05 grammar.

Frozen copy of the exp05/bench steering layer plus the three conditions of
exp06-a, which ask whether the arbitration failure observed in bench v0.2
(15B and 4B models never raise the budget under INFEASIBLE) is a problem of
information or of action:

  C0 baseline  -- the exp05 semantic prompt, unchanged.
  C1 info      -- same grammar, an imperative block spelling out what
                  INFEASIBLE means and which actions are available.
  C2 structure -- "escalate" becomes a first-class declaration, and
                  re-emitting an unchanged declaration under INFEASIBLE is
                  rejected as a sentence (the exp05 min_power guardrail
                  lifted to the arbitration level).
...
INFEASIBLE_IMPERATIVE_SECTION = """
## MANDATORY: what to do when status is INFEASIBLE
INFEASIBLE is not a warning, it is a proof: the optimizer has certified that
your declared constraints CANNOT be satisfied within the current power budget.
The optimizer has already done everything it can. Regulation will not help.

Re-emitting the same declaration changes NOTHING. The system will stay
INFEASIBLE for the rest of the episode and the policy will never be met.
Only YOU can resolve it, by changing the declaration itself:
  (a) raise "P_total" so the constraint becomes reachable -- this is usually
      the right action when the policy demands a performance level;
  (b) relax the declared constraint (lower its "min");
  (c) change the objective family.
Doing nothing is not an option. If the status line says INFEASIBLE, your
response MUST differ from your previous declaration.
"""

ESCALATE_GRAMMAR_ONLY = """
...
enforce cannot be met with what you have.
"""

REJECTION_SECTION = """
The system REJECTS a declaration that leaves everything unchanged while the
status is INFEASIBLE: under INFEASIBLE you must either escalate, or relax a
constraint, or change the objective. A rejected declaration is discarded, the
previous one is kept, and you are told why -- so an unchanged re-emission
simply wastes the call.
"""

ESCALATE_GRAMMAR_SECTION = ESCALATE_GRAMMAR_ONLY + REJECTION_SECTION


ESCALATE_IMMEDIATE_SECTION = """
## How an escalation is actuated
Ordinary "P_total" commands are smoothed (applied = 0.5*command + 0.5*current),
because a budget command is a servo signal. An ESCALATION is not a servo
signal but a discrete request, so it is applied IN FULL on the next step:
"escalate" with request R sets the budget cap to exactly R.
Ask for the amount you actually believe is needed, not a cautious step
towards it.
"""


def system_prompt(condition: str) -> str:
    """condition: 'semantic' | 'plain' | 'C0' | 'C1' | 'C2' | 'C3'.

    C0/C1/C2 are the exp06-a arbitration conditions; C0 is an alias of
    'semantic' so the baseline is bit-identical to bench v0.2.
    """
    if condition in ("C0", "semantic"):
        return system_prompt_base() + HISTORY_SECTION + STATUS_SEMANTIC_SECTION
    if condition == "C1":
        return (system_prompt_base() + HISTORY_SECTION
                + STATUS_SEMANTIC_SECTION + INFEASIBLE_IMPERATIVE_SECTION)
    if condition == "C2":
        return (system_prompt_base() + HISTORY_SECTION
                + STATUS_SEMANTIC_SECTION + ESCALATE_GRAMMAR_SECTION)
    if condition == "C3":
        return (system_prompt_base() + HISTORY_SECTION
                + STATUS_SEMANTIC_SECTION + ESCALATE_GRAMMAR_SECTION
                + ESCALATE_IMMEDIATE_SECTION)
    # exp06-a ablation: the grammar without the rejection rule.
    if condition == "A1":            # grammar only (EMA on, no rejection)
        return (system_prompt_base() + HISTORY_SECTION
                + STATUS_SEMANTIC_SECTION + ESCALATE_GRAMMAR_ONLY)
    if condition == "A3":            # grammar + immediate actuation, no rejection
        return (system_prompt_base() + HISTORY_SECTION
                + STATUS_SEMANTIC_SECTION + ESCALATE_GRAMMAR_ONLY
                + ESCALATE_IMMEDIATE_SECTION)
    if condition == "plain":
...
def parse_response(raw: str):
    ...
    # exp06 C2: an escalation on P_total is just another way of asking for a
    # budget, so it lands on the same knob and is clamped the same way. It is
    # kept separate in the return value so the analysis can tell an explicit
    # escalation apart from a bare P_total edit.
