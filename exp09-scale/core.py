...
def compute_objective(mi: torch.Tensor, lam: torch.Tensor, spec: ObjectiveSpec,
    ...
    if spec.family == "min_power":
        # Normalized so the gradient scale matches the other families. The
        # divisor is deliberately a constant and not the budget in force:
        # dJ/dlambda_i = -2 lambda_i / c, so a constant c leaves the descent
        # rate independent of both the cap and N, whereas c = P_total would
        # make it fall off as 1/N. The fixed point is the same either way --
        # c only rescales the multiplier -- so this is a choice about the
        # dynamics, not about what is being optimized.
...
def optimize_step(lam: torch.Tensor, h_re: torch.Tensor, spec: ObjectiveSpec,
    ...
    if al is not None and al.tau_ch_bits is not None:
        mi_bits = mi * NATS2BITS_CORE
        g_ch = al.tau_ch_eff - mi_bits  # vector, violation when positive
        mu_ch = torch.tensor(al.mu_ch, dtype=torch.float32, device=dev)
        pen_ch = torch.clamp(mu_ch + al.rho * g_ch, min=0.0)
        L = L - ((pen_ch ** 2 - mu_ch ** 2).sum()) / (2.0 * al.rho)
...
class AugLagState:
    ...
    mu: float = 0.0
    tau_ch_bits: float | None = None       # per-channel floor: MI_i >= tau_ch
    margin_ch_bits: float = 0.1            # per-channel chance-constraint margin
    mu_ch: np.ndarray = field(default_factory=lambda: np.zeros(N))
    rho: float = 1.0
    rho_max: float = 10.0   # keep the penalty from stiffening the dynamics
    rho_growth: float = 1.5
    mu_max: float = 20.0
    g_prev: float | None = None
    g_ch_prev: float | None = None
    # A declared sum-rate floor is extensive: a shortfall of a given
    # fraction is N times larger in bits at N channels than at one. The
    # penalty gradient is shared out over the channels, so the correction
    # applied per channel grows with N and overshoots. Dividing the
    # violation by N states the same constraint per channel -- g = 0 is the
    # same boundary -- and leaves the correction intensive. Set to False to
    # reproduce the extensive behaviour.
    per_channel_violation: bool = True
    ...
    @property
    def tau_ch_eff(self) -> float | None:
        if self.tau_ch_bits is None:
            return None
        return self.tau_ch_bits + self.margin_ch_bits

    def set_channel_constraint(self, tau_ch_bits: float | None):
        """Declare / update the uniform per-channel MI floor (bits)."""
        if tau_ch_bits is None:
            self.tau_ch_bits = None
            self.mu_ch = np.zeros(N)
            self.g_ch_prev = None
        else:
            self.tau_ch_bits = float(np.clip(tau_ch_bits, 0.0, 1.95))
    ...
    def set_constraint(self, tau_bits: float | None):
        ...
        if tau_bits is None:
            ...
            # just under the N-channel ceiling of 2 bits each; at N = 8 this
            # is 15.5, the value that was hard-coded before N was swept
            self.tau_bits = float(np.clip(tau_bits, 0.0, SR_CEILING_FRACTION
                                          * 2.0 * N))

    def dual_update(self, sr_bits_meas: float,
                    mi_bits_meas: np.ndarray | None = None) -> float:
        """Outer-iteration multiplier update from measured sum rate and,
        when a per-channel constraint is declared, per-channel MI (bits).

        Returns the worst signed constraint value (positive = violated).
        PHR updates with the standard safeguards; rho grows only while the
        violation fails to shrink (noise-floored).
        """
        g_worst = 0.0
        if self.tau_bits is not None:
            g = self.tau_eff - sr_bits_meas
            if self.per_channel_violation:
                g /= N
        ...
        if self.tau_ch_bits is not None and mi_bits_meas is not None:
            g_ch = self.tau_ch_eff - mi_bits_meas  # vector, positive = violated
            self.mu_ch = np.clip(self.mu_ch + self.rho * g_ch, 0.0, self.mu_max)
            g_max = float(g_ch.max())
            if (self.g_ch_prev is not None and g_max > 0.08
                    and g_max > 0.9 * self.g_ch_prev):
                self.rho = min(self.rho * self.rho_growth, self.rho_max)
            self.g_ch_prev = g_max
            g_worst = max(g_worst, g_max)
        return g_worst
...
def kkt_residuals(sr_bits_meas: float, lam_now: torch.Tensor,
                  ...
                  al: AugLagState,
                  mi_bits_meas: np.ndarray | None = None) -> dict:
    ...
    if al.tau_bits is not None:
        r_feas = max(r_feas, max(0.0, al.tau_bits - sr_bits_meas))
        r_comp += al.mu * abs(al.tau_eff - sr_bits_meas)
    if al.tau_ch_bits is not None and mi_bits_meas is not None:
        g_ch = al.tau_ch_bits - mi_bits_meas
        r_feas = max(r_feas, float(np.clip(g_ch, 0.0, None).max()))
        r_comp += float((al.mu_ch * np.abs(al.tau_ch_eff - mi_bits_meas)).sum())
    gap_bound = (al.mu + float(al.mu_ch.sum())) * r_feas + r_stat
...
def classify_status(res: dict, al: AugLagState, prev_status: str) -> str:
    ...
    infeasible = ((al.tau_bits is not None
                  ...
                  or (al.tau_ch_bits is not None
                      and float(al.mu_ch.max()) >= 0.9 * al.mu_max
                      and res["r_feas"] > EPS_FEAS))
    if infeasible:
