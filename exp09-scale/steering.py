"""System 2 plumbing, with the channel count left open.

A copy of bench/steering.py in which every place the prompt names the
number of sub-channels is a template slot, so the same grammar can be put
to a model at N = 8 and at N = 64 without any other change. The worked
examples stay eight-long -- writing a sixty-four-element example would
dominate the prompt -- and a single line tells the model how many entries
its own vectors must carry. Whether that line is enough is the question
run_scale_llm.py asks.
"""
...
NOTE: the two examples above are written for 8 sub-channels. This system has
N={n}, so "weights" and "targets" must each carry exactly {n} numbers.
"""
...
def system_prompt(condition: str, n: int | None = None) -> str:
    """condition: 'semantic' (signals explained) or 'plain' (signals bare).

    n defaults to the module-level N, which run_scale_llm.py rebinds.
    """
    # a plain replace, not .format(): the prompt is full of JSON braces
    s = SYSTEM_PROMPT_BASE.replace("{n}", str(N if n is None else n))
    s += HISTORY_SECTION
